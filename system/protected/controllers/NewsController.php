...
class NewsController extends AccessController {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions' => array('admin', 'del', 'create', 'update', 'delete', 'page', 'addpage', 'updatepage'),
                'users' => array('@'),
    ...
    public function actionAddpage() {
        ...
        if (isset($_POST['News'])) {
            ...
            if (isset($_POST['News']['tag']) && $_POST['News']['tag'])
                $model->tag = $_POST['News']['tag'];
    ...
    public function actionUpdatepage($id) {
        ...
        if (isset($_POST['News'])) {
            ...
            $model->updated_datetime = date('Y-m-d H:i:s', time());
            $model->alias = News::model()->convertLink($model->title);
    ...
    public function actionDel($id) {
        ...
        if (isset($news) && $news) {
            ...
            if($news->is_page == 1) {
                $menu = Menu::model()->findByAttributes(array('page_id' => $news->id));
                if(isset($menu) && $menu)
                    $menu->delete();
            }
